...
class ExpediaGroup(GenericMethods):
    ...
    def get_main_info(self, id, url):
        ...
        return results | temp_amenities | {"listing_url": url}
    ...
    def get_policies(self, id):
        ...
        try:
            ...
            for p in self.hotel_policies:
                ...
                temp_policies[p.replace(" ", "_").lower()] = ', '.join(self.get_elements(required_xpath))
            time.sleep(0.5)
            return {"group_id": id} | temp_policies
        except:
            return {}
    ...
    def get_important_information(self, id):
        ...
        try:
            for val in self.extra_info:
                ...
                temp_extra[val.replace(" ", "_").lower()] = ', '.join(self.get_elements(required_xpath))
            return {"group_id": id} | temp_extra
        except:
            return temp_extra
        ...
        try:
            ...
            for bed in all_beds:
                ...
                for val in self.room_features:
                    ...
                    temp_bed[val.replace(" ", "_").lower()] = ', '.join(self.get_elements(required_xpath))
    ...
    def combilne_all_sets(self, id, file_path, url):
        main = self.get_main_info(id=id, url=url)
        policies = self.get_policies(id=id)
        extra = self.get_important_information(id=id)
        faqs = self.get_faqs(id=id)
        beds = self.get_beds(id=id)
        reviews = self.get_reviews(id=id)
        ...
        if os.path.exists(file_path):
            with pd.ExcelWriter(file_path, engine="openpyxl", mode='a', if_sheet_exists='overlay') as writer:
                main_df.to_excel(writer, sheet_name="Data File", index=False, header=False, startrow=writer.sheets["Data File"].max_row)
                reviews_df.to_excel(writer, sheet_name="Review File", index=False, header=False, startrow=writer.sheets["Review File"].max_row)
                beds_df.to_excel(writer, sheet_name="Bed File", index=False, header=False, startrow=writer.sheets["Bed File"].max_row)
                policies_df.to_excel(writer, sheet_name="Policy File", index=False, header=False, startrow=writer.sheets["Policy File"].max_row)
                faqs_df.to_excel(writer, sheet_name="FAQ's File", index=False, header=False, startrow=writer.sheets["FAQ's File"].max_row)
                extra_df.to_excel(writer, sheet_name="More Information", index=False, header=False, startrow=writer.sheets["More Information"].max_row)
        else:
            with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
                main_df.to_excel(writer, sheet_name="Data File", index=False)
                reviews_df.to_excel(writer, sheet_name="Review File", index=False)
                beds_df.to_excel(writer, sheet_name="Bed File", index=False)
                policies_df.to_excel(writer, sheet_name="Policy File", index=False)
                faqs_df.to_excel(writer, sheet_name="FAQ's File", index=False)
                extra_df.to_excel(writer, sheet_name="More Information", index=False)
...
for id, url in enumerate(urls_list, start=group_id):
    ...
    bot.combilne_all_sets(id=id, file_path=file_path, url=url)
